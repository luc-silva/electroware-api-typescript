...
describe("User Controller - Register: ", () => {
  ...
  test("Server should not create a user without email", async () => {
    ...
    expect(response.status).toBe(400);
    expect(response.body).toEqual("Campo email inválido."); //this should be a message object in the future
  });
  ...
  test("Server should not create a user without first name", async () => {
    ...
    expect(response.status).toBe(400);
    expect(response.body).toEqual("Campo nome inválido.");
  });
  ...
  test("Server should not create a user without password", async () => {
    ...
    expect(response.status).toBe(400);
    expect(response.body).toEqual("Campo senha inválido.");
  });
});
